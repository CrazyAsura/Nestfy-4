...
import { Anchor, Eye, EyeOff, ArrowLeft, ArrowRight, UserPlus, Check } from 'lucide-react';
...
const STEPS = ['Identity', 'Business', 'Contact', 'Security'];

export default function SailorRegisterPage() {
  ...
  const nextStep = async () => {
    ...
    if (step === 1) fieldsToValidate = ['cnpj', 'companyName'];
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      const res = await authService.registerSailor({
        name: data.name,
        email: data.email,
        cnpj: data.cnpj,
      ...
      dispatch(setError('Registration failed. CNPJ or email may already exist.'));
    }
  };
  ...
  const inputClass = (hasError: boolean) =>
    `w-full bg-secondary/40 border rounded-lg px-4 py-3 text-sm font-mono text-foreground placeholder:text-muted-foreground/50 outline-none focus:ring-2 focus:ring-sailor-amber/30 ${hasError ? 'border-red-500' : 'border-border'}`;
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground flex items-center justify-center overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute top-[-30%] right-[-20%] w-[70%] h-[70%] bg-[radial-gradient(ellipse_at_center,rgba(255,184,0,0.06),transparent_60%)] pointer-events-none" />
        ...
        <div className="auth-card-sailor rounded-2xl p-8 md:p-10 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-col items-center gap-3 mb-6">
              ...
              <h1 className="text-xl font-black uppercase tracking-wider text-foreground">Create Merchant Account</h1>
            </div>

            {/* Step Indicator — Sailor amber */}
            <div className="step-indicator mb-8 px-2">
              {STEPS.map((label, idx) => (
                <React.Fragment key={label}>
                  <div
                    className={`step-dot ${idx === step ? 'active' : idx < step ? 'completed' : ''}`}
                    style={idx === step ? { borderColor: '#FFB800', color: '#FFB800', boxShadow: '0 0 12px rgba(255,184,0,0.3)' } : idx < step ? { borderColor: '#FFB800', background: '#FFB800', color: '#000' } : {}}
                  >
                    {idx < step ? <Check size={12} /> : idx + 1}
                  </div>
                  {idx < STEPS.length - 1 && (
                    <div className={`step-line ${idx < step ? 'completed' : ''}`} style={idx < step ? { background: '#FFB800' } : {}} />
                  )}
                </React.Fragment>
              ))}
            </div>
            ...
            <div className="flex justify-between px-1 mb-6">
              {STEPS.map((label, idx) => (
                <span key={label} className={`text-[8px] font-mono uppercase tracking-wider ${idx === step ? 'text-sailor-amber font-bold' : 'text-muted-foreground'}`}>
                  {label}
                </span>
              ))}
            </div>
            ...
            {error && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4 p-3 rounded-lg border border-red-500/30 bg-red-500/5 text-red-500 text-xs font-mono">{error}</motion.div>
            )}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
              <AnimatePresence mode="wait">
                {step === 0 && (
                  <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input type="text" placeholder="Captain Ahab" {...register('name', { required: 'Name is required' })} className={inputClass(!!errors.name)} />
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input type="email" placeholder="merchant@nestfy.com" {...register('email', { required: 'Email is required', pattern: { value: /^\S+@\S+\.\S+$/, message: 'Invalid email' } })} className={inputClass(!!errors.email)} />
                ...
                {step === 1 && (
                  <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold font-mono">CNPJ</label>
                      <input type="text" placeholder="00.000.000/0000-00" {...register('cnpj', { required: 'CNPJ is required' })} className={inputClass(!!errors.cnpj)} />
                      {errors.cnpj && <span className="text-[10px] text-red-500 font-mono">{errors.cnpj.message}</span>}
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold font-mono">Company Name</label>
                      <input type="text" placeholder="Nestfy Smart Solutions" {...register('companyName', { required: 'Company name is required' })} className={inputClass(!!errors.companyName)} />
                      {errors.companyName && <span className="text-[10px] text-red-500 font-mono">{errors.companyName.message}</span>}
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold font-mono">Business Description <span className="text-muted-foreground/50">(optional)</span></label>
                      <textarea placeholder="What does your business offer?" {...register('businessDescription')} rows={3} className={`${inputClass(false)} resize-none`} />
                    </div>
                  </motion.div>
                )}
                ...
                {step === 2 && (
                  <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input type="tel" placeholder="+55 (00) 00000-0000" {...register('phone', { required: 'Phone is required' })} className={inputClass(!!errors.phone)} />
                  ...
                  <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        <input type={showPassword ? 'text' : 'password'} placeholder="Min 8 chars, 1 uppercase, 1 number" {...register('password', { required: 'Password is required', minLength: { value: 8, message: 'Min 8 characters' }, pattern: { value: /^(?=.*[A-Z])(?=.*\d)/, message: '1 uppercase + 1 number required' } })} className={`${inputClass(!!errors.password)} pr-10`} />
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        <input type={showConfirm ? 'text' : 'password'} placeholder="Re-enter password" {...register('confirmPassword', { required: 'Please confirm', validate: (val) => val === watch('password') || 'Passwords do not match' })} className={`${inputClass(!!errors.confirmPassword)} pr-10`} />
                        <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-sailor-amber transition-colors">
              ...
              <div className="flex gap-3 mt-2">
                ...
                {step < STEPS.length - 1 ? (
                  <button type="button" onClick={nextStep} className="flex-1 py-3 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 bg-sailor-amber text-black hover:shadow-[0_0_30px_rgba(255,184,0,0.4)]">
                    Next <ArrowRight size={12} />
                  </button>
                ) : (
                  <button type="submit" disabled={loading} className="flex-1 py-3 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-50 bg-sailor-amber text-black hover:shadow-[0_0_30px_rgba(255,184,0,0.4)]">
            ...
            <p className="mt-6 text-center text-xs text-muted-foreground font-mono">
              ...
              <Link href="/auth/sailor/login" className="text-sailor-amber hover:underline font-bold">Sign in</Link>
            </p>
          </div>
        </div>
      </motion.div>
