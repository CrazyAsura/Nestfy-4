...
export default function ProductDetailPage({ params }: PageProps) {
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-black text-neon-blue font-mono text-xs tracking-widest uppercase">
        <div className="flex flex-col items-center gap-3">
          ...
          <span>{t('products.syncingCore')}</span>
        </div>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background text-foreground font-mono text-xs tracking-widest uppercase flex-col gap-6">
        ...
        <span>{t('products.hardwareNotSynced')}</span>
        <Link href="/">
          <CyberButton variant="outline">{t('products.returnStore')}</CyberButton>
        </Link>
      </div>
    );
  }
  ...
  const getLocalizedProduct = (prod: Product) => {
    ...
    const longDescription = t(`products.${pid}.longDescription`);
    
    // Localized features array check
    let features = prod.features;
    const langDict: any = translations[language];
    if (langDict && langDict.products && langDict.products[pid] && Array.isArray(langDict.products[pid].features)) {
      features = langDict.products[pid].features;
    }

    // Localized specs array check
    let specs = prod.specs;
    if (langDict && langDict.products && langDict.products[pid] && Array.isArray(langDict.products[pid].specs)) {
      specs = langDict.products[pid].specs;
    }
    ...
    return {
      ...
      longDescription: longDescription !== `products.${pid}.longDescription` ? longDescription : prod.longDescription,
      features,
      specs,
    } as Product;
  };
  ...
  const getLocalizedCategory = (cat: string) => {
    ...
    const localized = t(`products.categories.${slug}`);
    return localized !== `products.categories.${slug}` ? localized : cat;
  };
  ...
  const handleAddToCart = () => {
    ...
    if (isMobileViewport) {
      let toastMsgLocal = '';
      if (language === 'pt') {
        toastMsgLocal = `${lProd.name} adicionado ao deck!`;
      } else if (language === 'zh') {
        toastMsgLocal = `已将 ${lProd.name} 加入装备栏！`;
      } else {
        toastMsgLocal = `Added ${lProd.name} to deck!`;
      }
      setToastMsg(toastMsgLocal);
      setToastOpen(true);
    } else {
      setIsCartOpen(true);
    }
      ...
      <ThemeProvider theme={dynamicMobileTheme}>
        <Box sx={{ 
          ...
          <Container maxWidth="xs" sx={{ px: 2, pt: 3 }}>
            ...
            {/* Tagline */}
            <Chip 
              label={getLocalizedCategory(lProd.category)} 
              size="small" 
              color="primary" 
              sx={{ 
                fontSize: '0.65rem', 
            ...
            <Card sx={{ 
              ...
              <CardContent sx={{ py: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                <Box sx={{ position: 'absolute', top: 8, left: 8, fontFamily: 'monospace', fontSize: '0.6rem', color: 'text.secondary' }}>
                  SYS_TELEMETRY: {lProd.id.toUpperCase()}_v9.42
                </Box>
            ...
            {/* Features check list */}
            <Typography variant="subtitle2" sx={{ fontWeight: 'bold', mb: 1.5, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {t('products.keyIntegrations')}
            </Typography>
            <Stack spacing={1.5} sx={{ mb: 4 }}>
              {lProd.features.map((feat, fidx) => (
    ...
    <div className="relative min-h-screen bg-background text-foreground transition-colors duration-300 overflow-hidden cyber-grid">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 w-full items-start">
          ...
          <div className="lg:col-span-5 w-full">
            <GlassCard animate={false} className="border border-border">
              <div className="relative w-full h-80 bg-secondary/25 rounded border border-border flex flex-col items-center justify-center overflow-hidden">
                <div className="absolute inset-0 cyber-grid-dense opacity-25" />
                <div className="absolute top-0 left-0 w-full h-full bg-linear-to-t from-background via-transparent to-transparent z-10" />
                ...
                <Box sx={{ position: 'absolute', top: 12, left: 40, fontFamily: 'monospace', fontSize: '8px', color: 'text.secondary' }}>
                  SYS_MODEL_ID: {lProd.id.toUpperCase()}_v9.42_STABLE
                </Box>
                ...
                <div className="z-20 flex items-center gap-2 border border-primary/20 bg-primary/5 px-3 py-1 rounded font-mono text-[9px] text-primary tracking-widest uppercase">
                  <Terminal size={10} />
                  {t('products.localSystemConnected')}
                </div>
              </div>
            </GlassCard>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <span className="text-[10px] tracking-widest text-primary dark:text-neon-blue uppercase border border-border dark:border-neon-blue/20 bg-primary/5 dark:bg-neon-blue/5 px-2.5 py-0.5 rounded font-black">
                  {getLocalizedCategory(lProd.category)}
                </span>
                <div className="flex items-center gap-1.5 text-zinc-500 text-xs">
                  <Star size={12} className="text-yellow-500 fill-yellow-500" />
                  <span>{lProd.rating} (5 {t('products.reviewsCount')})</span>
                </div>
              </div>
            ...
            <GlassCard animate={false} className="border border-border p-5 bg-secondary/10">
              <div className="flex justify-between items-center gap-6 flex-wrap">
                <div className="flex flex-col">
                  <span className="text-[9px] uppercase tracking-wider text-zinc-500 font-bold">{t('products.syncPayloadPrice')}</span>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 text-xs text-zinc-500">
                    ...
                    <span>{t('products.inStock')} ({lProd.stock} units)</span>
                  </div>
                  <CyberButton variant="primary" onClick={handleAddToCart} className="w-48">
                    {t('products.syncToDeck')}
                  </CyberButton>
                </div>
              </div>
            </GlassCard>
            ...
            <div className="flex flex-col gap-3">
              <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 font-mono">
                {t('products.hardwareSpecsMatrix')}
              </h3>
            ...
            <div className="flex flex-col gap-3">
              <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 font-mono">
                {t('products.coreSystemIntegrations')}
              </h3>
      ...
      <footer className="border-t border-border bg-background/50 py-12 relative z-10 text-center text-xs tracking-wider text-zinc-500 font-mono uppercase flex flex-col gap-4">
        <div className="flex justify-center gap-6 flex-wrap text-[10px]">
          <Link href="/about" className="hover:text-primary transition-colors">{t('footer.aboutUs')}</Link>
          ...
          <Link href="/cookies" className="hover:text-primary transition-colors">{t('footer.cookiesPolicy')}</Link>
          <Link href="/terms" className="hover:text-primary transition-colors">{t('footer.termsOfUse')}</Link>
        </div>
